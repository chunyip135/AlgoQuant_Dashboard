...
def full_df_pipeline(api):
	...
	#last_trading_date = cal_ls[cal_ls[cal_ls <= tdy_date].index[-3]].strftime('%Y-%m-%d')
	last_trading_date = pd.Timestamp(cal_ls[cal_ls[cal_ls == tdy_date].index -2].values[0]).strftime('%Y-%m-%d')
